...
S3Adapter.prototype.uploadFile = function (file, callback) {
	...
	this.options.generateFilename(file, 0, function (err, filename) {
		...
		var fileStream = null, params = {};

		var isVideo =  file.mimetype.indexOf('video') === 0;
		var isAudio =  file.mimetype.indexOf('audio') === 0;
		if (isVideo || isAudio) {
			...
			debug('Uploading audio / video file %s', filename);
			...
			sharp(localpath)
				...
				.toBuffer((err, data, info) => {
						...
						console.log('SHARP RESIZED INFO: ', info);
						file = Object.assign(file, info);
						...
						debug('Uploading image file %s', filename);
						self.s3Client.upload(params, function (err, data) {
							if (err) return callback(err);
							file.filename = filename;
							file.etag = data.ETag;
